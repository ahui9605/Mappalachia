...
namespace Mappalachia
{
    ...
    public partial class FormMaster : Form
    {
        ...
        static void SetIsDrawing(bool isDrawing)
        {
            ...
            self.buttonDrawMap.Text = isDrawing ? "取消" : "更新地图";
        ...
        static void UpdateProgressBarText(string labelText, bool forceRefresh)
        {
            UpdateUIAsync(() =>
            {
                ...
                UpdateProgressBarLabelColor(labelText); // Call the method to update the color
        ...
        public static void UpdateProgressBar(double amount)
        {
            ...
            UpdateProgressBar(amount, amount == 0 ? "加载完毕" : self.labelProgressBar.Text);
        }
        ...
        static void UpdateProgressBarLabelColor(string labelText)
        {
            // Change the label background color based on the labelText.
            if (labelText.Contains("取消") || (labelText == "Cancelled"))
            {
                self.labelProgressBar.BackColor = Color.Red;
            }
            else if (labelText.Contains("完毕"))
            {
                self.labelProgressBar.BackColor = Color.Green;
            }
            else
            {
                self.labelProgressBar.BackColor = Color.Yellow; // Default color
            }
        }
        ...
        void UpdateHeatMapResolution(bool reDraw)
        {
            ...
            switch (SettingsPlotHeatmap.resolution)
            {
                ...
                case 1024:
                    ...
                    break;
                default:
                    Notify.Error("不支持的热力图分辨率 " + SettingsPlotHeatmap.resolution);
                    break;
            }
        ...
        void UpdateTopographColorBands(bool reDraw)
        {
            ...
            switch (SettingsPlotTopograph.colorBands)
            {
                ...
                default:
                    ...
                    Notify.Error("不支持的Topograph颜色带数量。默认为" + SettingsPlotTopograph.colorBands);
        ...
        void UpdateMapBackgroundSettings(bool reDraw)
        {
            ...
            switch (SettingsMap.background)
            {
                ...
                default:
                    throw new NotSupportedException("不支持的风格类型" + SettingsMap.background);
            }
        ...
        void UpdateLegendStyle(bool reDraw)
        {
            ...
            switch (SettingsMap.legendMode)
            {
                ...
                default:
                    throw new NotSupportedException("未知的图例样式" + SettingsMap.legendMode);
            }
        ...
        void WarnWhenLVLINotSelected()
        {
            ...
            if (!enabledSignatures.Contains("LVLI"))
            {
                ...
                foreach (string signatureToWarn in DataHelper.typicalLVLIItems)
                {
                    ...
                    {
                        Notify.Info(
                            "您的搜索结果可能受到限制，因为您在类型的筛选下没有选中 '战利品'。\n" +
                            "在《辐射76》中，许多有价值的物品通常被通用地归类为 '战利品'。\n" +
                            "如果您找不到您要找的东西，请确保启用它并重新搜索。");
        ...
        bool WarnWhenAllFiltersBlank()
        {
            ...
            {
                Notify.Info(
                    "未找到搜索结果，因为您已禁用了该类型的所有筛选\n" +
                    "您必须至少启用每个类型的一个筛选器才能找到任何内容");
                return true;
            }
        ...
        void UpdateSearchResultsGrid()
        {
            ...
            foreach (MapItem mapItem in searchResults)
            {
                gridViewSearchResults.Rows.Add(
                    ...
                    mapItem.weight == -1 ? "未知" : mapItem.weight.ToString(),
                    mapItem.count,
                    mapItem.spaceName,
        ...
        void ButtonSearchStandard(object sender, EventArgs e)
        {
            ...
            UpdateProgressBar(0.25, "搜索中...", true);
            ...
            UpdateProgressBar(0.50, "正在绘制界面...", true);
        ...
        void ButtonAddToLegend(object sender, EventArgs e)
        {
            ...
            UpdateProgressBar($"正在添加 {totalItems} 项...", true);

            if (totalItems == 0)
            {
                Notify.Info("请首先从搜索结果中选择物品");
                return;
            }
            ...
            if (totalRejectedItems > 0)
            {
                ...
                {
                    message += "有些来自于选择的区域之外";
                }

                if (rejectedItemsOtherSpace.Count > 0)
                {
                    message += "\n\n只有属于当前选定区域，搜索结果中的物品才能进行绘图 " +
                        "要仅在所选区域中查找结果，请确保 '搜索设置 > 在所有区域中搜索' 处于关闭状态";
                }
        ...
        void ButtonRemoveFromLegend(object sender, EventArgs e)
        {
            ...
            UpdateProgressBar($"移除 {selectedRowsCount} 项..", true);
        ...
        void GridViewLegend_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                {
                    Notify.Warn("\"" + editedCell.Value + "\" 必须是正整数");
        ...
        void GridViewSearchResults_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (hoveredCell.Value.ToString() == "未知")
                {
                    hoveredCell.ToolTipText = "由于多种可能的原因，Mappalachia 无法提供一个固定的数字来描述此实体的生成几率";
                }
            }
        ...
        void GridViewLegend_CellMouseEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (column.HeaderText != "图例组")
            {
                return;
            }

            string legendGroupTooltip = "图例组允许您将物品分组在一起，以便它们以相同的颜色和/或形状表示";
